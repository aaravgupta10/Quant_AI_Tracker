...
# Calculate active positions & All-Time Invested Capital
cash_balance = 0.0
net_invested = 0.0
positions = {}
...
# --- GLOBAL LIVE EQUITY & P&L CALCULATION ---
...
all_time_pnl = live_equity - net_invested
pnl_percentage = (all_time_pnl / net_invested * 100) if net_invested > 0 else 0.0
...
with tab1:
    ...
    # Adding All-Time Profit right next to the Live NAV
    col1.metric("Live Total Asset Value", f"₹{live_equity:,.2f}") 
    col2.metric("All-Time Profit / Loss", f"₹{all_time_pnl:,.2f}", f"{pnl_percentage:.2f}%")
    ...
    if metrics_data:
        ...
        st.divider()
        st.markdown("### Performance vs Nifty 50 (Base 100)")
        
        df_metrics = pd.DataFrame(metrics_data)
        df_metrics['date'] = pd.to_datetime(df_metrics['date'])
        
        # Plot Unit NAV (which ignores cash injections) vs Nifty
        if 'unit_nav' in df_metrics.columns:
            with st.spinner("Fetching Benchmark Data..."):
                ...
                # Merge and Normalize Nifty to start at 100
                ...
                # Plot
                fig_curve = px.line(chart_df, x='date', y=['Portfolio (Unit NAV)', 'Nifty 50'], title='True Growth: Portfolio vs Benchmark')
                fig_curve.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', yaxis_title="Growth (Base ₹100)", legend_title_text="Legend")
                st.plotly_chart(fig_curve, use_container_width=True)
        else:
            st.warning("⚠️ Run quant_engine.py in terminal to generate Unit NAV data.")
...
with tab2:
    ...
    if not active_positions:
        ...
        if live_equity <= 0:
            st.error("Cannot calculate rebalance: Live market data unavailable.")
        else:
            ...
            with st.spinner(f"Calculating matrix for {target_ticker}..."):
                try:
                    ...
                    if target_ticker not in returns.columns or '^NSEI' not in returns.columns: st.error("Insufficient market data.")
                        ...
                        if live_equity > 0:
                            weights = {t: (live_prices.get(t, 0.0) * active_positions[t]) / live_equity for t in current_tickers if t in returns.columns}
    ...
    if dcf_ticker:
        with st.spinner(f"Pulling fundamental data for {dcf_ticker}..."):
            try:
                ...
                if current_price == 0 or shares_out == 0: st.error("Missing essential data.")
                else:
                    ...
                    if st.button("Calculate Intrinsic Value"):
                        ...
                        intrinsic_value = (pv_fcfs + pv_terminal_value + total_cash - total_debt) / shares_out
