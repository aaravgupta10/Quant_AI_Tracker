...
with tab1:
    ...
    if metrics_data:
        ...
        st.warning("⚠️ No metrics found. Run quant_engine.py to populate.")
...
with tab2:
    ...
    st.write("Test a new stock against your current portfolio before you buy it.")
    ...
    if st.button("Run Matrix Analysis"):
        if not current_tickers:
            st.warning("Your portfolio is empty. Buy some stocks first to run correlation!")
        elif not target_ticker:
            st.warning("Please enter a ticker.")
        else:
            with st.spinner(f"Downloading 1-year data and calculating matrix for {target_ticker}..."):
                ...
                with col_macro:
                    ...
                    if nifty_corr > 0.8: st.write("📉 **Index Hugger:** You move tightly with the broader market.")
                    elif nifty_corr < 0.3: st.write("🛡️ **Decoupled:** Your path is unique from the market.")
                    else: st.write("⚖️ **Moderate:** Standard market correlation.")
                        
                with col_target:
                    st.info(f"**{target_ticker} vs Current Portfolio:** {port_corr:.2f}")
                    if port_corr > 0.7: st.error("DANGER: Moves identically to your portfolio. Minimal diversification.")
                    elif port_corr > 0.4: st.warning("MODERATE: Some overlap. Acceptable, but monitor exposure.")
                    elif port_corr > -0.2: st.success("EXCELLENT: Largely uncorrelated. True diversification.")
                    else: st.success("HEDGE: Moves opposite to your portfolio. Defensive shield.")
...
with tab3:
    ...
    st.write("Record your real-world transactions here to instantly update your ledger.")
    ...
    with st.form("trade_form", clear_on_submit=True):
        ...
        with col_qty:
            quantity = st.number_input("Quantity (Shares or 1 for Cash)", min_value=0.01, value=1.0)
        with col_price:
            price = st.number_input("Price (Per share or Total cash amount)", min_value=0.01, value=100.0)
        ...
        if submit_button:
            if not ticker_input:
                ...
                # Format logic just like the CLI
                ...
                try:
                    ...
                    pull_vault_data.clear() # Force Streamlit to grab the fresh data
...
with st.sidebar:
    ...
    st.info("To recalculate your advanced Risk Metrics (Alpha/Beta) after logging a trade, run the engine:")
    st.code("python quant_engine.py", language="bash")